...
def resnet(units, num_stage, filter_list, num_classes, data_type, bottle_neck=True, bn_mom=0.9, workspace=512, memonger=False):
    ...
    body = mx.sym.BatchNorm(data=data, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='bn_data')
    ...
    body = mx.sym.Activation(data=body, act_type='relu', name='output_relu1')

    return body

def resnet_ssd(units, num_stage, filter_list, num_classes, data_type, bottle_neck=True, bn_mom=0.9, workspace=512, memonger=False, **kwargs):
    nms_thresh, force_suppress, nms_topk = kwargs['nms_thresh'], kwargs['force_suppress'], kwargs['nms_topk']

    label = mx.symbol.Variable(name="label")

    body = resnet(units, num_stage, filter_list, num_classes, data_type, bottle_neck=True, bn_mom=0.9, workspace=512, memonger=False)    
    
    graph = mx.viz.plot_network(body.get_internals(), node_attrs={"shape":'rect',"fixedsize":'false'})
    graph.render('./img/misc/resnet', cleanup=True)
    
    # specific parameters for ResNet network
    ...
    graph = mx.viz.plot_network(out, node_attrs={"shape":'rect',"fixedsize":'false'})
